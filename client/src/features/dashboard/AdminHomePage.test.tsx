...
describe('AdminHomePage', () => {
  ...
  it('links to the academy settings, academics settings, and profile pages', async () => {
    ...
    expect(screen.getByRole('link', { name: '관리자 계정' })).toHaveAttribute('href', '/admin/settings/admins');
    expect(screen.getByRole('link', { name: '역할·권한 관리' })).toHaveAttribute('href', '/admin/settings/roles');
    expect(screen.getByRole('link', { name: '보고서' })).toHaveAttribute('href', '/admin/reports');
