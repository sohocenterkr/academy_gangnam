...
import { z } from 'zod';
import { formatTimeKST, getNowKSTISOString, getTodayKST } from '@shared/kst';
...
export function createCheckInRouter(deps: CheckInRouterDeps): Router {
  const router = Router();
  const searchLimiter = createRateLimitMiddleware({ windowMs: 60_000, max: 20 });
  const confirmLimiter = createRateLimitMiddleware({ windowMs: 60_000, max: 20 });
    ...
    try {
      created = await db.transaction(async (tx) => {
        ...
        await tx.insert(checkInChangeLogs).values({
          ...
          action: 'create',
          beforeData: null,
          afterData: { checkInAt: row.checkInAt, source: 'kiosk' },
          reason: null,
          adminId: null,
          createdAt: new Date(),
        });
      ...
      if (isUniqueViolation(error, 'check_ins_student_date_active_unique')) {
        ...
        res.status(409).json({
          error: {
            ...
            message: `이미 ${existing ? formatTimeKST(existing.checkInAt) : ''}에 등원했습니다.`,
