...
describe('admins routes', () => {
  ...
  it('rejects concurrent updates atomically — only one of two simultaneous PATCHes succeeds', async () => {
    ...
    await Promise.all([request(app).get('/api/admins').set('Cookie', cookie), request(app).get('/api/admins').set('Cookie', cookie)]);

    const [first, second] = await Promise.all([
      request(app)
        .patch(`/api/admins/${created.body.data.id}`)
        .set('Cookie', cookie)
        .send({ name: '이름A', expectedUpdatedAt }),
      request(app)
        .patch(`/api/admins/${created.body.data.id}`)
        .set('Cookie', cookie)
        .send({ name: '이름B', expectedUpdatedAt }),
    ]);
